...
import {Box,Typography,Modal,ButtonGroup,Button,Checkbox, FormControlLabel,Alert} from '@mui/material'
...
import {ad2landscape } from '../../assets';
...
export default function CustomModal({isOpen}) {
  ...
  try {
    const res = await client.post('/promo2',{
       ...values
    });
    if(res.data.success){
    ...
    const handleOk = async() =>{
      try {
        const response = await client.patch('/updatepromo2/'+localStorage.getItem('id'),{
         ...social
        })
  ...
  return (
    <div>
      ...
      >
        <Box sx={style} >
        <Box component="img" src={ad2landscape} sx={{width:"100%"}} alt=''/>
